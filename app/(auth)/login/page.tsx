    ...
    <div className="flex h-screen w-screen overflow-hidden bg-white">
      ...
      <div className="relative flex w-full flex-col overflow-y-auto bg-white lg:w-1/2">
        ...
        <div className="w-full max-w-sm">
          {/* Heading */}
          <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 translate-x-10">
            Welcome Back
          </h1>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        {/* Floating cards / UI mockups */}
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 p-12">
          <div className="w-full max-w-xs rounded-2xl bg-white/10 p-5 backdrop-blur-sm border border-white/20 shadow-xl">
            <div className="mb-3 flex items-center gap-3">
              <div className="h-8 w-8 rounded-full bg-purple-400/60" />
            ...
            <div className="mt-4 flex gap-2">
              <div className="h-7 w-20 rounded-lg bg-purple-400/50" />
          ...
          <div className="flex w-full max-w-xs gap-3">
            {[
              { label: 'Revenue', value: '$1.2M', color: 'from-purple-400/40 to-purple-600/40' },
              { label: 'Orders', value: '+893', color: 'from-pink-400/40 to-pink-600/40' },
            ].map((s) => (
              <div
                key={s.label}
                className={`flex-1 rounded-xl bg-linear-to-br ${s.color} p-4 backdrop-blur-sm border border-white/20`}
              >
          ...
          <div className="w-full max-w-xs rounded-2xl bg-white/10 p-5 backdrop-blur-sm border border-white/20">
            ...
            <div className="flex items-end gap-2 h-16">
              {[40, 65, 45, 80, 55, 90, 70].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-t-md bg-linear-to-t from-purple-400/70 to-pink-400/70"
                  style={{ height: `${h}%` }}
                />
              ))}
            </div>
          ...
          <div className="text-center">
            <h2 className="text-3xl font-extrabold text-white drop-shadow">Slooze</h2>
            <p className="mt-1 text-sm text-white/60">Commodities Management Platform</p>
          </div>
        </div>
      </div>
    </div>
